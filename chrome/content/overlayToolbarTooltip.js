...
var gmanager_ToolbarTooltip = new function()
{
  ...
  this._buildAccountDetails = function(aAccount)
  {
    ...
    el.setAttribute("value",
                    this.getFString("inbox-unread", [aAccount.inboxUnread]));
    elParent.appendChild(el);

    // Saved Drafts
    el = document.createElement("label");
    el.setAttribute("value",
                    this.getFString("saved-drafts", [aAccount.savedDrafts]));
    ...
    el = document.createElement("label");
    el.setAttribute("value",
                    this.getFString("spam-unread", [aAccount.spamUnread]));
    ...
    if (true) // TODO: Add preference to show/hide labels
    {
      ...
      for (var i = 0; i < labels.length; i++)
      {
        ...
        // TODO: Add preference to show/hide labels with unread mail
    ...
    el = document.createElement("label");
    el.setAttribute("value",
                    this.getFString("space-used",
                                    [aAccount.spaceUsed,
                                     aAccount.percentUsed,
                                     aAccount.totalSpace]));
    elParent.appendChild(el);
    ...
    if (aAccount.getBoolPref("toolbar-tooltip-show-snippets"))
    {
      ...
      if (snippets.length > 0)
      {
        ...
        for (var i = 0; i < Math.min(10, snippets.length); i++)
        {
          ...
          el.setAttribute("value",
                          gmanager_Utils.toUnicode(snippets[i].from) +
                          " > " +
                          gmanager_Utils.toUnicode(snippets[i].subject));
